...
1. Unit.cs を変更する前に、Unity Editor で Stage1, Stage2, Stage4, Stage5 を目視確認すること
2. 一度に複数の visual elements を変更しない。まずは HP text または selection ring のどちらか一方から手をつける
3. resource sprite pivot の変更は最後の手段とする
4. Battle logic / enemy AI / StageData / victory-defeat rules は触らない
5. このドキュメントの対象外である UnityManualTestChecklist.md も触らない
